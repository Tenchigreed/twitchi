...
class theTwitcher(QWidget):
    ...
    def setupUi(self):
        ...
        self.action = QAction("Fenetre")
        self.menu.addAction(self.action)
                
        self.quitApli = QAction("Quit")
        self.menu.addAction(self.quitApli)
    ...
    def setupConnection(self):
        ...
        self.quitApli.triggered.connect(QApplication([]).quit)
    ...
    def creditDef(self):    
        class creditClass(QWidget):
            ...
            def setupUi2(self):
                self.gridLayout = QGridLayout(self)
                self.resize(150,50)
                urlLink="<p>Auteur: <a href=\"twitch.tv/mrvietsky\">'MrVietsky'</a></p>\n<p>Developpeur: Mahieux(<a href=\"https://tenchigreed.fr\">'TenchiGreed'</a>) Dany</p>"
    ...
    def boutonOkDef(self):
        global username
        global URLLive
        usernameSpace = str(self.name.text())
        if usernameSpace[0] == ' ':
            username = usernameSpace[1:]
        elif usernameSpace[len(usernameSpace)-1] == ' ':
            username = usernameSpace[:len(usernameSpace)-1]
        else:
